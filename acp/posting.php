...
// Disallow Usernames
$lang = array_merge($lang, array(
	'ACP_DISALLOW_EXPLAIN'	=> 'Здесь вы можете управлять именами пользователей, запрещёнными для использования. Запрещённые имена могут содержать подстановочные знаки (*). Примечание: вы не можете запретить зарегистрированное имя. Сначала вам придётся удалить пользователя, а уже потом запретить освободившееся имя.',
	...
	'DISALLOWED_ALREADY'		=> 'Введённое имя не может быть запрещено. Оно может уже содержаться в списке запрещённых имен, или в списке автоцензора, или существует пользователь с таким именем.',
